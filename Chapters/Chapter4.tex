...
\begin{scheme}
    ...
    \caption{Synthesis of \refcmpd{ebs-thio.ph,ebs.ph}.}
...
We suspected that the benzimidazole proton may be competing with the amidic proton in the benzisothiazolinone ring, as it is substantially more acidic (pKa???), so we protected it using \emph{p}-methoxy benzyl chloride to give \cmpd{rhs-bromo.2py}$\cdot$PMB (\cref{sch:rhs-protection}).
...
This gave the desired coupling product \cmpd{ebs-thio-rhs}$\cdot$PMB as a mixture of isomers, of which we obtained a single crystal structure (\cref{sch:ebs-thio-rhs-synthesis} and \cref{fig:ebs-thio-rhs-pmb-xray}).
We were also able to prepare the benzisoselenazolinone derivative \cmpd{ebs-rhs}$\cdot$PMB as a pair of isomers.
...
\begin{scheme}
    ...
    \caption{Synthesis of \refcmpd{ebs-thio-rhs}$\cdot$PMB and \refcmpd{ebs-rhs}$\cdot$PMB.}
    \label{sch:ebs-thio-rhs-synthesis}
\end{scheme}

\begin{figure}
    \includegraphics[width = 0.8\linewidth]{Figures/ebs-thio-rhs-pmb-xray.pdf}
    \caption{X-ray crystal structure of one isomer of \refcmpd{ebs-thio-rhs}$\cdot$PMB}
...
\begin{figure}[ht]
    ...
    \caption{X-ray crystal structure of \refcmpd{ebs-rhs.ph}. The water is disordered over 3 sites, with each proton having an occupancy of 2/3.}
