...
\cmpd*{m2pb}
Initial targets \cmpd{ebs-rhs,bim-ebs,ebs-ebs} were identified as likely minor groove binders, based on their similarity to the bis-benzimidaozles previously studied in the group.
Of these lead molecules, the 2-pyridyl derivative \cmpd{m2pb} was the best radioprotector.
...
\begin{figure}
    ...
    \replacecmpd{bim-ebs}
    \replacecmpd{ebs-ebs}
    \includegraphics[scale=0.8]{Figures/targets.eps}
    \caption{Lead bis-benzimidaozles \refcmpd{m2pb} and initial target compounds \refcmpd{ebs-rhs,bim-ebs,ebs-ebs}.}
    \label{fig:targets}
\end{figure}

The inital route to this molecule is shown in \cref{sch:ebs-rhs-synthesis-1}, which involved first synthesising the benzisoselenazolinone moiety, then constucting the bisbenzimidazole in a condensation and reductive dehydrocyclisation.
...
\begin{figure}
    ...
    \includegraphics[scale=0.8]{Figures/ebs-rhs-diselenide.eps}

    GET FROM JOHNNY
    %\includegraphics[width=0.5\linewidth]{Figures/diselenide-benzimidazole-2py-xray.png}
    \caption{X-ray crystal structure of \refcmpd{ebs-rhs-diselenide}.}
...
We therefore devised another scheme in which the benzisoselenazolinone ring is formed last (\cref{sch:ebs-rhs-synthesis-2}).
...
Rather than investigate other pathways to this compound (such as the use of coupling agents), we decided to simplify the system to a phenyl ring (to afford \cmpd{ebs-rhs.ph}), as this had previously been shown to bind to DNA.
...
This was coupled to the bis-electrophile \cmpd{dichloride} to afford the desired benzisoselenazolinone-benzimidaozle \cmpd{ebs-rhs.ph}.

A number of routes to functionalised benzisoselenazolinones were envisaged, \emph{via} various intermediates.
We were inspired by the Ullman-type chemistry exploited by \textcite{Bhabak2010} in their selenocyclisation, and by the observation that the amidic proton in \cmpd{ebs.h} goes on to react further to furnish the tetracyclic compound \cmpd{tetracycle} (see \cref{sec:crystengcomm1}).
...
A number of test reactions were performed using the benzisothiazolinone \cmpd{ebs-thio.h}, which is commercially available and inexpensive.
Typical Ullman conditions were used (high temperature, DMF solvent, high \ce{CuI} catalyst loading) to initally couple \cmpd{ebs-thio.h} with bromobenzene to afford \cmpd{ebs-thio.ph} in moderate yield (\cref{sch:ebs-thio-ph-synthesis}).
We were also able to prepare ebselen \cmpd{ebs.ph} via this route in similar yield.
...
\begin{scheme}
    \replacecmpd{dichloride}
    \replacecmpd{ebs.h}
    \includegraphics[scale=0.8]{Figures/ebs-h-synthesis.eps}
    \caption{Synthesis of \refcmpd{ebs.h}.}
    \label{sch:ebs-h-synthesis}
\end{scheme}

\begin{scheme}
    \replacecmpd{ebs-thio.h}
    \replacecmpd{ebs-thio.ph}
    TO DO
    %\includegraphics[scale=0.8]{Figures/ebs-thio-ph-synthesis.eps}
    \caption{Synthesis of \refcmpd{ebs-thio.ph}.}
    \label{sch:ebs-thio-ph-synthesis}
\end{scheme}

1-Bromo-3,4-dinitrobenzene was reduced to the diamine using tin (II) chloride, (??? CHECK THIS) then reacted with 2-pyridyl carboximidate hydrochloride to afford the benzimidazole \cmpd{rhs-bromo.2py} in excellent yield.
Unfortunately this gave no detectable product in the coupling reaction.
We suspected that the benzimidazole proton may be competing with the amidic proton in the benzisothiazolinone ring, as it is substantially more acidic (pKa???), so we protected it using \emph{p}-methoxy benzyl chloride (\cref{sch:rhs-protection}).
This afforded two distinct isomers \cmpd{rhs-bromo-pmb.2py-a,rhs-bromo-pmb.2py-b} from the two freely interconverting tautomers of \cmpd{rhs-bromo.2py}.
...
We therefore prepared the phenyl derivative \cmpd{rhs-bromo.ph} by the same method, and protected it similarly.
This gave the desired isomeric coupling products \cmpd{ebs-thio-rhs-pmb.a,ebs-thio-rhs-pmb.b}, of which we obtained a single crystal structure (\cref{sch:ebs-thio-rhs-synthesis}).
We were also able to prepare the benzisoselenazolinone derivative as the isomers \cmpd{ebs-rhs-pmb.a,ebs-rhs-pmb.b}.
...
\begin{scheme}
    ...
    \replacecmpd{ebs-ebs.ph}
    \caption{Synthesis of bis-benzisoselenazolinone Hoechst analogue \refcmpd{ebs-ebs.ph}.}
