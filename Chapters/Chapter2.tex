...
Spinning at 12~kHz instead of 10~kHz showed that the signals at 834.69 and 867.02~ppm are the true chemical shifts, and the fact that there are two signals show that there are indeed two Ch-bonded systems in the asymmetric unit.
...
In the first spectrum, no magic angle spinning was used, and the lineshape is characteristically broadened due to chemical shift anisotropy and dipolar coupling.
Note the poor S/N ratio due to the broad peak.
Assuming constant power is deposited in the sample, sharper lines will give improved S/N ratio as the energy is focused in narrower bands.
...
There exist programs (such as XXX\autocite{???}) which can fit an experimental lineshape or sideband manifold affording the principal values of the tensor, however there is clearly a trade-off between obtaining a good S/N ratio and having enough sidebands within the limits of the signal for a meaningful fit.
...
The resulting spectra are presented in \ref{fig:77se-ssnmr-ebs}, and the extracted principal values of the chemical shift tensor are presented in \ref{77se-ssnmr-ebs-csa}.
In order to verify our results, we also \emph{calculated} the chemical shielding tensors in order to compare the principal values, and also determine the likely orientation of the tensors with respect to the rest of the molecule.

\cmpd{ebs.4ome}$\cdot$DMAP differs from the other structures, in that it crystallises in the space group $P2_1/c$, whereas \cmpd{ebs.ph}$\cdot$DMAP and \cmpd{ebs.4no2}$\cdot$DMAP both crystallise in $P\bar{1}$.
