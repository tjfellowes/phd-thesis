...
These interactions appear to be primarily due to orbital overlap as opposed to electrostatic or dispersion mediated effects, as evidenced by lengthening of the antipodal \ce{Se-N} bond, and computational analysis, which is consistent with findings in related systems by Cockroft et al.\autocite{Pascoe2017}
...
The benzisoselenazolinone derivatives \cmpd[merge=false,compress=false]{ebs,ebs.{4no2,4cn,4cf3,4br,4co2et,4me,4ome,4oet}} were prepared by the reaction of substituted anilines with a selenenyl chloride intermediate \cmpd{diselenide}, ultimately derived from anthranilic acid (\cref{sch:ebs-synthesis}).
...
\begin{table}
  ...
  \tablefit{\begin{tabular}{lllllllll}
    \toprule
    Complex & r(\ce{N \cdots Se}) & r(\ce{Se-N_{1}}) & r(Se--C\textsubscript{1}) & r(N\textsubscript{1}--C(O)) & $ \angle $(N$ \cdots $\ce{Se-N_{1}}) & $ \angle $(C\textsubscript{para}$ \cdots $\ce{N \cdots Se}) & $\rho_{\mathrm{BCP}}$(\ce{Se\cdots N}) & $\nabla^{2}(\rho_{\mathrm{BCP}})$(\ce{Se\cdots N}) \\
...
The Hammett substituent parameter $ \Delta $ for a given substituent is determined from the ionization equilibrium of the parent carboxylic acid.
...
Closely linked to the substituent constant $ \Delta $ are the values of $\sigma^{+}$ and $\sigma^{-}$, which can be used when mesomeric effects have a strong influence on the property being studied.
...
As the magnitude of the $ \Delta $-hole (Ch-bond donor ability) is \emph{increased} by an electron withdrawing substituent, the Ch-bond acceptor ability of the carbonyl is \emph{decreased}.
...
\begin{table}
  ...
  \tablefit{\begin{tabular}{llllllll}\toprule
    ...
    \cmpd{py.morph} & 2.68                 & -0.13(2)  & 2.397(8)  & 0.8708& 0.035(10)& 1.9673(34)& 0.7260 \\
    ...
    \bottomrule 
  \end{tabular}}\label{tab:hammett-results}
\end{table}

The results of the linear regression analysis for the Hammett plots in \cref{fig:hammett-dmap,fig:hammett-pyrrol,fig:hammett-morph} are given in \cref{tab:hammett-results}.
Qualitatively we can see a link between the strength of the base used and the gradient of the line of best fit.
...
\begin{figure}
    ...
    \caption[Solid state IR spectrum of \refcmpd{ebs.4oet}$ \cdot $DMAP.]{Solid state IR spectrum of \refcmpd{ebs.4oet}$ \cdot $DMAP.\@ The spectrum of the pure \refcmpd{ebs.4oet} is shown in gray, which contains only one molecule in the asymmetric unit.}\label{fig:ebs-4oet-dmap-ir}
\end{figure}
...
\cmpd{ebs.4ome}$ \cdot $DMAP differs from the other structures, in that it crystallises in the space group $P2_1/c$, whereas \cmpd{ebs}$ \cdot $DMAP and \cmpd{ebs.4no2}$ \cdot $DMAP both crystallise in $P\bar{1}$.
...
In order to convert these chemical shielding tensors $ \Delta $ into chemical shifts $ \Delta $, we must reference them to a standard $\sigma_{\textrm{ref}}$.
\begin{equation}
...
In the absence of the lone pair of the pyridyl nitrogen, the $ \Delta $-hole strongly deshields the selenium, therefore the large $\delta_{11}$ component is roughly aligned with it.
...
This is particularly important for the chemical shift of the non-complexed heterocycle, as such a ``free'' $ \Delta $-hole would not be observed in any condensed phase (it is always filled by solvent in solution, or the carbonyl of an adjacent molecule in the solid).
...
This verifies that the tensor is oriented correctly in the crystal, and that the observed shielding is due to the approach of the lone pair into the $ \Delta $-hole.
...
For single site binding (a valid approximation for these systems, as the single $ \Delta $-hole is likely to out-compete all other interactions), the dissociation constant can be expressed as
...
We initially injected an equimolar mixture of DMAP and ebselen \cmpd{ebs} in methanol, and isolated an ion of m/z 398.08 a.m.u.
...
Furthermore, \ce{N+ -H\cdots O} hydrogen bonds are very strong (in excess of 15~kcal/mol)\autocite{Emsley1980}, which is likely to out compete Ch-bonding, with estimated bond enthalpies of <~10~kcal/mol in the gas phase.
...
\begin{figure}
    ...
    \caption[Negative mode ESI of \refcmpd{ebs}$ \cdot $isonicotinate.]{Dissociation of the complex \refcmpd{ebs}$ \cdot $isonicotinate under CID.}\label{fig:neg-esi-ms}
\end{figure}
...
\ce{^{1}H} NMR (400~MHz, \ce{\textit{d}6}-DMSO) $ \Delta $ ppm
7.29--7.38 (m, 1 H),
...
\ce{^{1}H} NMR (400~MHz, \ce{CDCl3}) $ \Delta $ ppm 8.30 (2H, d, \textit{J} = 9.07~Hz), 8.14 (1H, d, \textit{J} = 7.82~Hz), 7.92 (2H, d, \textit{J} = 9.07~Hz), 7.75--7.66 (2H, m), 7.52 (1H, t, \textit{J} = 6.8~Hz).
...
\ce{^1H} NMR (500~MHz, DMSO-\textit{d6}) $ \Delta $ 8.08 (d, \textit{J} = 7.8~Hz, 1H), 8.02 (d, \textit{J} = 8.7~Hz, 2H), 7.92 (dd, \textit{J} = 0.4, 8.0~Hz, 1H), 7.88 (d, \textit{J} = 8.6~Hz, 2H), 7.71 (t, \textit{J} = 7.0~Hz, 1H), 7.50 (t, \textit{J} = 7.3~Hz, 1H), 4.32 (q, \textit{J} = 7.2~Hz, 2H), 1.33 (t, \textit{J} = 7.1~Hz, 3H).
...
\ce{^1H} NMR (400~MHz, DMSO-\textit{d6}) $ \Delta $ 8.06 (d, J = 8.0 Hz, 1H), 7.87 (d, J = 7.3 Hz, 1H), 7.66 (dt, J = 0.3, 7.4 Hz, 1H), 7.48 (d, J = 8.3 Hz, 2H), 7.46 (t, J = 7.3 Hz, 1H), 7.24 (d, J = 8.2 Hz, 2H), 2.31 (s, 3H).

\subsubsection{4-Methoxy ebselen \refcmpd{ebs.4ome}}
Light brown crystals, dec.\textsuperscript{\ref{fn:decompose}}, yield 67\%.

\ce{^1H} NMR (500~MHz, DMSO-\textit{d6}) $ \Delta $ 8.07 (d, J = 7.9 Hz, 1H), 7.88 (d, J = 7.6 Hz, 1H), 7.67 (t, J = 7.1 Hz, 1H), 7.49 (d, J = 8.4 Hz, 2H), 7.47 (t, J = 6.9 Hz, 1H), 7.01 (d, J = 8.6 Hz, 2H), 3.78 (s, 3H).

\subsubsection{4-Ethoxy ebselen \refcmpd{ebs.4oet}}
Light brown crystals, dec.\textsuperscript{\ref{fn:decompose}}, yield 67\%.

\ce{^1H} NMR (500~MHz, DMSO-\textit{d6}) $ \Delta $ 8.06 (d, J = 8.0 Hz, 1H), 7.87 (d, J = 7.5 Hz, 1H), 7.67 (t, J = 7.2 Hz, 1H), 7.48 (t, J = 3.2 Hz, 1H), 7.47 (d, J = 8.8 Hz, 2H), 6.99 (d, J = 8.8 Hz, 2H), 4.05 (q, J = 7.0 Hz, 2H), 1.34 (t, J = 6.9 Hz, 3H).
