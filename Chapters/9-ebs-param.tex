...
First synthesised in 1924, its unusual properties went more or less uninvestigated for more than 50 years.\autocite{Lesser1924}
...
This has been substantially addressed in the work of Orian \textit{et al.}, where extensive parametrization of a series of diaryl diselenides and diaryl ditellurides was performed, and they provided a methodology to extend this to general chalcogen-containing molecules.\autocite{Torsello2016}
...
This method has been successfully used to model anion binding through a Ch-bond in work performed by Beer \textit{et al.}, however the approach does not seem to have gained traction in wider circles.\autocite{Lim2017}
...
The ground state geometry of ebselen was optimised at the $ \omega $B97M-V/def2-TZVP level, followed by vibrational analysis to confirm the structure was minimized.\autocite{Chai2008,Weigend2005,Weigend2006}
...
The equilibrium distance/angle $ x_0 $ was fixed to the value from the optimised geometry.
...
There are thought to be three attractive energetic components which contribute to a $ \sigma $-hole interaction, namely, electrostatics, induction, and dispersion.\autocite{Bleiholder2006,Bleiholder2007,Pascoe2017}
...
SAPT(DFT) analyses were conducted using the Psi4 software package on geometries optimised at the $ \omega $B97M-V/def2-TZVP level, and the results are shown in \cref{fig:eda}.\autocite{Parrish2017}
...
We note that this does not necessarily contradict our earlier work, in which we have experimentally demonstrated a charge-transfer component to the interaction, as we have made no claim as to its relative magnitude with respect to electrostatics, only its presence and structural effects.

\subsection{Incorporation of pseudoatom}
With classical parameters for ebselen in hand, as well as theoretical assurance that the system can be adequately described using electrostatics, we began to optimise parameters for the pseudoatom representing the $ \sigma $-hole.
...
This is very computationally efficient, although it is an approximation to reality, as the center of the $ \sigma $-hole is actually slightly offset due to the potential from the aromatic ring ``spilling'' over onto the surface of the selenium (see \cref{fig:ebs-esp}).
The angle $\angle $(\ce{N-Se\cdots B}) is also slightly less than 180\degree{}, although in the other direction to the true location of the $ \sigma $-hole, an observation which we attribute to sterics.
A more accurate description of the $ \sigma $-hole would be the three-center \texttt{3fd} or \texttt{3fad} type, however this would introduce a performance penalty.
...
SOD1 (PDB \textbf{2C9V}) was chosen because of the availability of an atomic resolution structure, demonstrated evidence of ebselen binding, and its relatively small size.\autocite{Capper2018,Strange2006}
