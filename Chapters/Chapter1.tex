...
The crystal was comprised of long chains of monomers (figure \ref{fig:hassel-xray}), exhibiting a very short O\dots Br contact (2.71\AA, the sum of the VDW radii is 3.37\AA), and a slightly lengthened Br--Br bond (2.31\AA~vs 2.28\AA~in gaseous bromine).
...
Similar applications have also been found for these classes of noncovalent interactions, Ch-bonding in particular.\autocite{Fanfrlik2014,Garrett2016,Ho2016,Wonner2017,Mitchell2017,Benz2017,Biot2017,Ho2017}
...
As was briefly discussed in the introduction, all intermolecular (and indeed \emph{intra}molecular) forces are manifestations of the electromagnetic force.
...
\chapter{Simulating chalcogen bonding using molecular mechanics: A pseudoatom approach to model ebselen.}
...
Very recently it was identified as a compound of interest for the treatment of COVID-19, showing promising inhibition of the viral M\textsuperscript{pro} protease enzyme.\autocite{Jin2020}
...
The selenium-containing heterocycle is reductively opened to afford the free selenol \textbf{1a}, which is the active catalyst.
This is rapidly oxidised by ROS to a selenenic acid \textbf{2}, which is then reduced back to \textbf{1a} by glutathione (GSH) via a selenenyl sulfide \textbf{3}.
...
\begin{figure}
\centering
\includegraphics[scale=0.6]{Figures/ebs-cat-cycle.eps}
...
Electrostatic potentials were calculated using the \texttt{cubegen} program in the Gaussian suite, or \texttt{mol2cub} (https://github.com/tjfellowes/mol2cub).
...
The resulting data was truncated to within 5~kcal/mol of the equilibrium energy (at larger distances the surfaces were appreciably anharmonic), and this surface was fitted with a classical harmonic oscillator model (equation \ref{eqn:cho}) using the \texttt{nls} function in the R software package.\autocite{R}
The equilibrium distance/angle $x_0$ was fixed to the value from the optimized geometry.
Torsion angles were similarly scanned at the DFT and MM (with the torsion term set to zero) levels, and the difference between these surfaces was fitted using a periodic series truncated to the fourth order (equation \ref{eqn:dihe}).
...
The resulting parameters are presented in tables \ref{tab:cho-params} and \ref{tab:dihe-params}.
...
The electrostatic component generally refers to the interaction between two static (not distorted by each other) electric fields, which can be graphically represented by visualizing the electrostatic potential surfaces of the donor and acceptor moieties (figure \ref{fig:ebs-esp}).
...
There are numerous EDA schemes available such as KM-EDA, NEDA, and ALMO, however we chose to use symmetry-adapted perturbation theory (SAPT).\autocite{SAPT2020,Jeziorski1994}
...
Their structures are given in figure \ref{fig:complexes}.
SAPT(DFT) analyses were conducted using the Psi4 software package on geometries optimized at the $\omega$B97X-D/def2TZVP level, and the results are shown in figure \ref{fig:eda}.\autocite{Parrish2017}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/sapt-eda.png}
...
With these parameters in hand, we were able to construct electrostatic potential maps (figure \ref{fig:ebs-esp}), which show good qualitative agreement between the DFT and pseudoatom model potentials.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/mm-dft-esp.png}
...
Trajectories were performed at 300~K for the strongly bonded systems (\textbf{1}$\cdot$py and \textbf{1}$\cdot$DMAc), however the weaker complexes (\textbf{1}$\cdot$TMA and \textbf{1}$\cdot$DMS) tended to dissociate under these conditions.
...
Relevant parameters are given in figure \ref{fig:1.py-geom} and table \ref{tab:ebs-geom}, alongside DFT values for comparison.

\begin{figure*}
    \centering
    \includegraphics[width=\linewidth]{Figures/geom-distribution.png}
...
\begin{table*}
    \centering
    \begin{tabular*}{0.96\linewidth}{@{\extracolsep{\fill}}lllll}\hline
        Complex & r(\ce{Se\cdots B}) & $\angle$(\ce{N-Se\cdots B}) & $\angle$(lone pair) & Energy \\ & & & & (kcal/mol)\\\hline
        \textbf{1}$\cdot$py & 2.98~\AA~(2.775~\AA) & 169.8\degree~(176.2\degree) & 158.2\degree~(165.7\degree) & -8.376 (-7.093) \\
        \textbf{1}$\cdot$DMAc & 2.964~\AA~(2.786~\AA) & 166.3\degree~(172.4\degree) & 129.7\degree~(117.9\degree) & -10.351 (-7.551) \\
        \textbf{1}$\cdot$TMA & 3.091~\AA~(2.857~\AA) & 167.6\degree~(177.4\degree) & 100.71\degree~(113.6\degree) & -6.666 (-6.627) \\
        \textbf{1}$\cdot$DMS & 3.322~\AA~(3.265~\AA) & 165.0\degree~(177.4\degree) & 89.1\degree~(89.8\degree) & -4.541 (-5.646) \\
        \hline
    \end{tabular*}
    \caption{Median geometric parameters for complexes with \textbf{1}. DFT equilibrium values are given in brackets for comparison. DFT energies are derived from SAPT(DFT) (table S5). REDO}
    \label{tab:ebs-geom}
\end{table*}
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{Figures/melting.png}
...
Interestingly, the weaker complexes (\textbf{1}$\cdot$TMA and \textbf{1}$\cdot$DMS) are better described both in terms of geometry and energy.
...
A simulation of 2~ns at 300~K was then performed to assess the average binding geometry, which was found to exhibit a bifurcated Ch-bond between the expected Cys111 sulfur and the adjacent Ile113 backbone carbonyl (figure \ref{fig:sod1-ebs}).
...
\begin{figure*}
    \centering
    \includegraphics[width=0.45\linewidth]{Figures/sod1-ebs-a.png}
    \includegraphics[width=0.45\linewidth]{Figures/sod1-ebs-b.png}
