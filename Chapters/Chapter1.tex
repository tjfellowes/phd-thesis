...
Torsion angles were similarly scanned at the DFT and MM (with the torsion term set to zero) levels, and the difference between these surfaces was fitted using a periodic series truncated to the fourth order (\cref{eqn:dihe}).
...
The crystal was heated to 300~K over 2~ns using a Berendsen thermostat and barostat, and the density was calculated to be 1.667~g/cm\textsuperscript{3}, which compares very favourably to the experimental density of 1.646~g/cm\textsuperscript{3}.\autocite{Dupont1990StructuresII}
Five random molecules were then deleted to create voids (crystal defects) that act as nucleation sites for melting.
...
These were conducted over 5~ns using a Berendsen thermostat and Parrinello-Rahman barostat.
The geometries of the simulated crystal was inspected at the end of the trajectory, and the transition from ordered crystal to disordered liquid was apparent at 472--473~K???, which compares well to the experimental melting point of 456~K.
We note that the slight overestimation may be explained by our modification of the Lennard-Jones parameters for the carbonyl oxygen atom, which may lead to artificially close packing and overstated electrostatic attraction.

This procedure was repeated using the non-pseudoatom model, which afforded a melting point of XXX.
The pseudoatom clearly represents a major improvement when simulating the solid state structure of ebselen.
