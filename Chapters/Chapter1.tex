...
In diamond, the carbon atoms are actually tetrahedral in form due to the bonding electron density, which reduces the space group symmetry and causes the 222 reflection to be present, though very weak.\autocite{Bragg1920}
...
Quantum crystallography is generally concerned with extracting a wavefunction from diffraction data, rather than using a wavefunction to improve a crystallographic model.
...
Even with the power of modern computers, this is not an easy task, and quantum crystallography remains an active and exciting are of research.

Perhaps the most powerful approach is that of multipole refinement using the formalism developed by Hansen and Coppens.\autocite{Hansen1978}
Atomic coordinates are determined from high angle refinement or neutron data.
Electron density is expanded as a series of spherical harmonics.
Can capture true electron density in a completely experimental (no assumptions) way.
