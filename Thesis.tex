...
\setmarginsrb{3.7cm}  % left margin 3.7cm 
             {1.5cm}  % top margin
             {2.7cm}  % right margin
             {3.0cm}  % bottom margin
             { 35pt}  % head height
...
\usepackage[backend=biber, style=chem-acs, autocite=superscript, doi=false]{biblatex}
...
\usepackage[stretch=35]{microtype}
\usepackage{placeins}
\usepackage{enumitem}
...
\listofcompounds{cll}  % Include a list of Abbreviations (a table of two columns)
{
...
\cmpdrow{rhs-bromo-2py.pmb}{rhs-bromo-2py_pmb.eps} \\
